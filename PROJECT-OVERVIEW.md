...
## 11. Monetization readiness (researched 2026-09-11)

Wave 1 is complete (auto, home-energy, sport, 28/28) and Wave 2 (health) is underway. Before continuing further into raw calculator count, a monetization-readiness check changes near-term priority.

**The current bottleneck is indexing, not content volume.** `site:manikalkulatori.lv` returns zero results as of this writing; the site is not indexed by Google at all yet, and no Google Search Console property is verified for the domain (no `google-site-verification` tag anywhere in the codebase). `robots.txt` and `sitemap.xml` are both correct and live (68 URLs), so the technical side is ready; what's missing is a Google account claiming the property and submitting the sitemap, which only the site owner can do. The domain is also only about three weeks old as of this note, which raises AdSense rejection risk for a few more weeks regardless of content quality. Building more calculators does not move this bottleneck; getting indexed does.

**AdSense** remains the eventual primary ad network (Latvia is fully supported, SEPA/EUR payouts) but should wait for some real indexing and organic traffic rather than being applied for immediately — a very young, unindexed domain is a common rejection reason independent of content quality.

**Two lower-barrier paths can start earlier, independent of AdSense's traffic/domain-age bar:**
- **Ezoic's "Access Now"** program has no minimum traffic requirement and can be joined immediately as an AdSense alternative or bridge.
- **Direct CPA/CPL affiliate programs for loans and leasing already exist and accept new publishers regardless of traffic volume**, since they pay per lead/contract, not per pageview: Elizings.lv (leasing, ~2% CPA), Latvijas Kredītu Centrs (loans, CPL €1.20–3.00, CPA €35–100), and the doaffiliate.net network (carrying BigBank, SMSCredit, Credit24 quick-loan programs). Insurance affiliate relationships (BTA, BALTA) appear to be broker/agency partnerships rather than self-serve affiliate links, so they need direct outreach rather than a signup form — lower priority than the loan/leasing programs.

**Build-order exception this creates:** the finance category (section 5, currently 0/14 built) was deliberately placed last in the original wave plan to avoid an SEO fight with kalkulatori.lv over generic queries. That reasoning still holds for *organic-ranking* purposes, but it doesn't apply to *affiliate* revenue, which doesn't need to outrank the incumbent — the page only needs to exist and eventually catch some traffic (including direct/referral, not only organic search). Building `patēriņa kredīta kalkulators` (item 17, P1, consumer loan cost, `/finanses/kredita-kalkulators`) ahead of the rest of the finance category is a deliberate exception to the gap-first wave order, made specifically because a ready, traffic-independent affiliate program exists for it today. The rest of the finance category (sections 13-26) still waits for its normal place in the wave order.
