...
export function calculateJavasDaudzums(input: JavasDaudzumaInput): JavasDaudzumaResult {
  ...
  const { unitsNeeded, wallAreaWithWasteM2 } = calculateKiegeluBlokuDaudzums({
  ...
  const wallVolumeM3 = wallAreaWithWasteM2 * (input.unitWidthMm / MM_PER_M);
  ...
  const mortarVolumeLiters = mortarVolumeM3 * LITERS_PER_M3;
